...
@description: GJR-GARCH volatility forecasting features for hybrid regime classification.
'''
...
def _build_mean_model(y: np.ndarray, cfg: GjrGarchConfig):
    ...
    if mean_kind == "zero":
        ...
        raise ValueError(f"Unsupported GJR-GARCH mean='{cfg.mean}'. Use 'zero' or 'constant'.")

    if cfg.vol.lower() not in {"gjr-garch", "gjrgarch", "gjr"}:
        raise ValueError(
            f"Unsupported volatility process vol='{cfg.vol}'. Use 'GJR-GARCH'."
        )
    model.volatility = GARCH(p=cfg.p, o=cfg.o, q=cfg.q)
...
def fit_gjrgarch(
    train_df: pd.DataFrame, cfg: GjrGarchConfig
    ...
    for ticker in train_df["ticker"].unique():
        ...
        if cfg.train_nobs_by_ticker is not None:
            ...
            if expected is not None and expected != len(td):
                print(
                    f"[gjrgarch] Warning: ticker={ticker} train rows mismatch "
        ...
        try:
            ...
            print(f"[gjrgarch] Warning: fit failed for {ticker}: {e}")
            fitted[ticker] = None
    return fitted


def make_gjrgarch_features(
    ...
    sigma_fwd_col = f"gjrgarch_sigma_fwd_h{h}"
    var_fwd_col = f"gjrgarch_var_fwd_h{h}"
    var_mean_col = f"gjrgarch_var_mean_h{h}"
    sigma_rms_col = f"gjrgarch_sigma_rms_h{h}"

    output_df["gjrgarch_sigma_t"] = np.nan
    output_df[sigma_fwd_col] = np.nan
    output_df[var_fwd_col] = np.nan
    if agg_kind != "last":
        output_df[var_mean_col] = np.nan
        output_df[sigma_rms_col] = np.nan
    output_df["gjrgarch_resid"] = np.nan
    output_df["gjrgarch_z"] = np.nan

    ann_sigma_factor = float(np.sqrt(252.0)) if cfg.annualize else 1.0
    ann_var_factor = 252.0 if cfg.annualize else 1.0
    min_obs_for_state = max(20, cfg.p + cfg.o + cfg.q + 5)
    ...
    for ticker in output_df["ticker"].unique():
        ...
        for i in range(min_obs_for_state - 1, n - h):
            try:
                ...
                if err in (1, 5, 10) or err % 50 == 0:
                    logger.warning("[gjrgarch] ticker=%s i=%s error=%r", ticker, i, e)
                if err > max_err:
                    raise RuntimeError(
                        f"[gjrgarch] Too many errors for ticker={ticker}: {err}/{len(ticker_df)}"
                    ) from e
                continue

        ticker_df["gjrgarch_sigma_t"] = sigma_t
        ...
        ticker_df["gjrgarch_resid"] = resid
        ticker_df["gjrgarch_z"] = z
